...
type Database struct {
	...
	mediaDB     *mongo.Collection // arcapi.medias — YouTube media file cache
...
func InitDatabase() error {
	...
	// The media cache collection lives in the "arcapi" database, mirroring tosu4
	arcapiDB := client.Database("arcapi")
	Instance = &Database{
		client:      client,
		DB:          db,
		...
		mediaDB:     arcapiDB.Collection("medias"),
