...
// Uses FallenApiUrl/FallenApiKey (API_URL2/API_KEY2) — the Fallen/onegrab API
// handles all non-YouTube platforms (Spotify, Apple Music, Deezer, SoundCloud, etc.).
func newApiData(query string) *apiData {
	return &apiData{
		Query:    strings.TrimSpace(query),
		ApiUrl:   strings.TrimRight(config.FallenApiUrl, "/"),
