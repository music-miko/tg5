...
// downloadTrack downloads a track using the appropriate backend.
// YouTube (audio + video) always goes through the Arc API / DB channel cache.
// All other platforms (Spotify, Apple Music, SoundCloud, Deezer, etc.) continue
// to use the original API gateway — behaviour is unchanged for those platforms.
func (a *apiData) downloadTrack(info utils.TrackInfo, video bool) (string, error) {
	// YouTube — always use Arc API (includes DB channel cache check internally)
	if info.Platform == utils.YouTube {
		yt := newYouTubeData(a.Query)
		return yt.downloadTrack(info, video)
	}

	// All other platforms — original API gateway path (Spotify, Apple, SC, Deezer …)
